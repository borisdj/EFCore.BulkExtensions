...
**OmitClauseExistsExcept** removes the clause from Merge statement, useful when need to active triggers even for same data. Also for come [sql collation](https://github.com/borisdj/EFCore.BulkExtensions/issues/641) small and capital letters are considered te same (case-insensitive) so if need update then it should be set to false.<br>
