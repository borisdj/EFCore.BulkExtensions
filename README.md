...
If we want Insert only new and skip existing ones in Db (InsertI_if_not_Exist) then use *BulkInsertOrUpdate* with config
...
[Spatial](https://docs.microsoft.com/en-us/sql/relational-databases/spatial/spatial-data-types-overview?view=sql-server-ver15) types, like Geometry, also supported and if  Entity has one it skips clause *EXIST ... EXCEPT* because not comparable.<br>
