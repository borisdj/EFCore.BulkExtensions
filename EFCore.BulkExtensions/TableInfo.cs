        ...
        protected void UpdateEntitiesIdentity<T>(IList<T> entities, IList<T> entitiesWithOutputIdentity)
        {
            if (BulkConfig.PreserveInsertOrder) // Updates PK in entityList
            {
                ...
                for (int i = 0; i < NumberOfEntities; i++)
                {
                    ...
                    // Like the id of the entity, a timestamp/rowversion is also generated by the SqlServer. 
                    // When SetOutputIdentity is set to true, entity's newly generated id as well its rowversion(if available) should be updated.
