...
public static class BatchUtil
{
    ...
    private static List<object> ReloadSqlParameters(DbContext context, List<object> sqlParameters)
    {
    ...
    public static (string, string, string, string, string, IEnumerable<object>) GetBatchSql(IQueryable query, DbContext context, bool isUpdate)
    {
        ...
        var (fullSqlQuery, innerParameters) = query.ToParameterizedSql();
    ...
    private static string GetSqlSetSegment(DbContext context, Type? updateValuesType, object? updateValues, List<string>? updateColumns, List<object> parameters)
    {
...
#pragma warning restore EF1001
    ...
    private static (string, string) SplitLeadingCommentsAndMainSqlQuery(string sqlQuery)
    {
            ...
            && !mainSqlQuery.StartsWith("SELECT", StringComparison.OrdinalIgnoreCase))
        {
            if (mainSqlQuery.StartsWith("--"))
            {
                ...
                {
                    _ = leadingCommentsBuilder.Append(mainSqlQuery[..(indexOfNextNewLine + Environment.NewLine.Length)]);
            ...
            if (mainSqlQuery.StartsWith("/*"))
            {
                ...
                {
                    _ = leadingCommentsBuilder.Append(mainSqlQuery[..(nextBlockCommentEndIndex + 2)]);
            ...
            if (selectIndex > 0)
            {
                _ = leadingCommentsBuilder.Append(mainSqlQuery[..selectIndex]);
    ...
    private static bool TryCreateUpdateBodyNestedQuery(BatchUpdateCreateBodyData createBodyData, Expression expression, MemberAssignment memberAssignment)
    {
        ...
        var (innerSql, innerSqlParameters) = innerQueryable.ToParameterizedSql();
